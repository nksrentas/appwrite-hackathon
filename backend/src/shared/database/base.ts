...
export interface GetUserInsightsResult {
  insights: Insight[];
}
...
export class ChallengeService {
  static async getChallenges(params: GetChallengesParams): Promise<GetChallengesResult> {
    return executeAppwriteOperation(async () => {
      const { active, category, limit = 50 } = params;
      ...
      if (active !== undefined) {
        ...
        if (active) {
          query.lessEqual('start_date', now).greaterThan('end_date', now);
        } else {
      ...
      query.orderDesc('$createdAt').limit(limit);
      
      const result = await databases.listDocuments(
        DATABASE_ID,
        COLLECTION_IDS.CHALLENGES,
        query.build()
      );
      
      const challenges = result.documents as unknown as Challenge[];
      
      logger.appwriteOperation('getChallenges', COLLECTION_IDS.CHALLENGES, undefined, true);
      
      return { challenges };
    }, 'getChallenges');
  }

  static async createChallenge(params: CreateChallengeParams): Promise<CreateChallengeResult> {
    return executeAppwriteOperation(async () => {
      const { challengeData } = params;
      ...
      logger.appwriteOperation('createChallenge', COLLECTION_IDS.CHALLENGES, result.$id, true);
      
      const challenge = result as unknown as Challenge;
      return { challenge };
    }, 'createChallenge');
  }
}
...
export class InsightService {
  static async getUserInsights(params: GetUserInsightsParams): Promise<GetUserInsightsResult> {
    return executeAppwriteOperation(async () => {
      const { userId, shown, limit = 25 } = params;
      
      const query = new QueryBuilder()
        .equal('user_id', userId);
      
      if (shown !== undefined) {
        query.equal('shown_to_user', shown);
      }
      ...
      const result = await databases.listDocuments(
        DATABASE_ID,
        COLLECTION_IDS.INSIGHTS,
        query.build()
      );
      
      const insights = result.documents as unknown as Insight[];
      
      logger.appwriteOperation('getUserInsights', COLLECTION_IDS.INSIGHTS, undefined, true);
      
      return { insights };
    }, 'getUserInsights');
  }

  static async createInsight(params: CreateInsightParams): Promise<CreateInsightResult> {
    return executeAppwriteOperation(async () => {
      const { insightData } = params;
      ...
      const result = await databases.createDocument(
        ...
        'unique()',
        insightData,
        PermissionHelper.userOwned(insightData.user_id)
      );
      
      logger.appwriteOperation('createInsight', COLLECTION_IDS.INSIGHTS, result.$id, true);
      
      const insight = result as unknown as Insight;
      return { insight };
    }, 'createInsight');
  }
}